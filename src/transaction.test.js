...
describe('Transaction', () => {
  ...
  it("should be able to update a transaction (add new outputs)", () => {
    ...
    transaction = transaction.update(senderWallet, nextRecipientAddress, nextAmount)

    const senderOutput = transaction.outputs.find(output => output.address === senderWallet.address)
    expect(senderOutput.amount).toEqual(- amount - nextAmount)

    const nextRecipientOutput = transaction.outputs.find(output => output.address === nextRecipientAddress)  
